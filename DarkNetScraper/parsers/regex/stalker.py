...
class DOT_Wallet(Item):
    ...
    def ss58_decode(address):
        ...
        if a[0] == 42:
            if len(a) == 32 + 1 + 2:
                address_bytes = a[:33]
                hash_bytes = hashlib.blake2b(address_bytes).digest()
                if a[33] == hash_bytes[0] and a[34] == hash_bytes[1]:
                    return address_bytes #Main net
                else:
                    # invalid checksum
                    return False
            else:
    ...
    def isvalid(address: str) -> bool:
        ...
        try:
            if re.search(dot_wallet_regex, address)[0] == address: 
                ret = DOT_Wallet.ss58_decode(address)[0] == 0
                return ret
            else:
                ret = False
        except:
